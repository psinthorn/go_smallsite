...
const (
	queryInsertRateType           = `insert into rate_types () values () returning id`
...
func (r *RateType) Create(rt RateType) (int, error) {
	...
	var id int
	err = dbConn.SQL.QueryRowContext(ctx, queryInsertRateType, rt.Title, rt.ShortName, rt.Description, rt.Status, rt.CreatedAt, rt.UpdatedAt).Scan(&id)
	if err != nil {
		return 0, err
	}
